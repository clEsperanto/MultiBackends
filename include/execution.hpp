...
static auto cudaDefines(const ParameterMap &parameter_list,
                        const ConstantMap &constant_list) -> const std::string {
  // @StRigaud TODO: write cuda Defines to transform ocl Kernel into compatible
  // cuda kernel See
  ...
  for (const auto &[key, value] : parameter_list) {
    if (std::hold_alternative<float>(value) || std
        : holds_alternative<int>(value)) {
      continue;
    }
    ...
    defines << "\n";
    defines << "\n#define IMAGE_SIZE_" << key << "_WIDTH " << width defines
            << "\n#define IMAGE_SIZE_" << key << "_HEIGHT " << height defines
            << "\n#define IMAGE_SIZE_" << key << "_DEPTH " << depth defines
            << "\n";
    ...
    defines << "\n";
    defines << "\n#define IMAGE_" << key << "_TYPE " << size_parameters << ""
            << arr.dtype() << "*";
    defines
...
static auto oclDefines(const ParameterMap &parameter_list,
                       const ConstantMap &constant_list) -> std::string {
  std::ostringstream defines;
  defines << "\n#define GET_IMAGE_WIDTH(image_key) IMAGE_SIZE_ ## image_key ## "
             "_WIDTH";
  defines << "\n#define GET_IMAGE_HEIGHT(image_key) IMAGE_SIZE_ ## image_key "
  ...
  for (const auto &[key, value] : parameter_list) {
    ...
    defines << "\n#define IMAGE_" << key << "_PIXEL_TYPE " << arr.dtype() << "";
    defines << "\n#define POS_" << key << "_TYPE " << pos_type;
    defines << "\n#define POS_" << key << "_INSTANCE(pos0,pos1,pos2,pos3) ("
            << pos_type << ")" << pos;
    defines << "\n";
    ...
    if (true) // @StRigaud TODO: introduce cl_image / cudaArray
    {
      ...
      defines << "\n#define READ_" << key << "_IMAGE(a,b,c) read_buffer" << ndim
              << "d" << arr.shortType()
              << "(GET_IMAGE_WIDTH(a),GET_IMAGE_HEIGHT(a),GET_IMAGE_DEPTH(a),a,"
                 "b,c)";
      defines << "\n#define WRITE_" << key << "_IMAGE(a,b,c) write_buffer"
              << ndim << "d" << arr.shortType()
              << "(GET_IMAGE_WIDTH(a),GET_IMAGE_HEIGHT(a),GET_IMAGE_DEPTH(a),a,"
                 "b,c)";
    } else {
      std::string img_type_name;
      if (key.find("dst") != std::string::npos ||
          key.find("destination") != std::string::npos ||
      ...
      defines << "\n#define READ_" << key << "_IMAGE(a,b,c) read_image"
              << prefix << "(a,b,c)";
      defines << "\n#define WRITE_" << key << "_IMAGE(a,b,c) write_image"
              << prefix << "(a,b,c)";
    }

    // define size information
    defines << "\n";
    defines << "\n#define IMAGE_SIZE_" << key << "_WIDTH "
            << std::to_string(arr.width());
    defines << "\n#define IMAGE_SIZE_" << key << "_HEIGHT "
            << std::to_string(arr.height());
    defines << "\n#define IMAGE_SIZE_" << key << "_DEPTH "
            << std::to_string(arr.depth());
    defines << "\n";
  }
...
static auto execute(const DevicePtr &device, const KernelInfo &kernel_func,
  ...
  // @StRigaud TODO: call execution based on backend, warning dealing with
  // void** and void* is not safe
